...
function figureOutTreesFromSlope(
  right: number,
  down: number,
  input: Array<string>
): number {
...
async function solveForFirstStar(
  ...
  report(
    `Solution ${test ? 'for test input' : '1'}:`,
    figureOutTreesFromSlope(3, 1, inputAsArray).toString()
  )
}
...
async function solveForSecondStar(
  ...
  report(
    `Solution ${test ? 'for test input' : '2'}:`,
    (one * two * three * four * five).toString()
  )
}

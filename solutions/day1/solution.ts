  ...
  for (i = 0; i < inputAsArray.length; i++) {
    for (j = 0; j < inputAsArray.length; j++) {
      if (Number(inputAsArray[i]) + Number(inputAsArray[j]) === 2020) {
        ...
        return report(
          'Solution 1:',
          (+inputAsArray[i] * +inputAsArray[j]).toString()
        )
      }
    }
  }
      ...
      for (k = 0; k < inputAsArray.length; k++) {
        if (
          Number(inputAsArray[i]) +
            Number(inputAsArray[j]) +
            Number(inputAsArray[k]) ===
          2020
        ) {
          console.log(
            `Found: ${inputAsArray[i]}, ${inputAsArray[j]}, and ${inputAsArray[k]}`
          )
          return report(
            'Solution 1:',
            (+inputAsArray[i] * +inputAsArray[j] * +inputAsArray[k]).toString()
          )
        }
      }
    }

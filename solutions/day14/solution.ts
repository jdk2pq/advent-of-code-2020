  ...
  input.forEach(line => {
    if (line.startsWith('mask = ')) {
      ...
      if (valueAsBinary.length < 36) {
        valueAsBinary = `${[...new Array(36 - valueAsBinary.length)]
          .map(() => '0')
          .join('')}${valueAsBinary}`
      }
      if (debug) {
      ...
      if (debug) {
        ...
        console.log(
          `writing ${parseInt(afterMask, 2)} to memory location ${memLocation}`
        )
      }
  ...
  input.forEach(line => {
    if (line.startsWith('mask = ')) {
      ...
      if (memLocationAsBinary.length < 36) {
        memLocationAsBinary = `${[...new Array(36 - memLocationAsBinary.length)]
          .map(() => '0')
          .join('')}${memLocationAsBinary}`
      }
      if (debug) {
      ...
      const combinations = getAllCombinations(changeablePortionStr).map(
        v => `${beginning}${v}`
      )
      if (debug) {
...
async function solveForFirstStar(
  ...
  report(
    ...
    solve(inputAsArray as string[], debug).toString()
  )
...
async function solveForSecondStar(
  ...
  report(
    ...
    solve2(inputAsArray as string[], debug).toString()
  )

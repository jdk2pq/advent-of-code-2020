...
function getBagCombos(inputAsArray: Array<any>): {[key: string]: {[key: string]: number}} {
  return inputAsArray.reduce((acc, line) => {
    ...
    const objContains: {[key: string]: number} = contains.reduce((acc, c) => {
      let color = c.split(/bag|bags/)[0].split(' ').slice(1).join(' ').trim()
...
async function solveForFirstStar(input: string, inputAsArray: Array<any>, test: boolean, debug: boolean) {
  ...
  report(`Solution 1${test ? ' (for test input)' : ''}:`, available.length.toString())
}
